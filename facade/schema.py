...
class Query:
    ...
    reservations: list[types.Reservation] = strawberry_django.field(resolver=queries.reservations, description="List of all reservations.")
    myreservations: list[types.Reservation] = strawberry_django.field(resolver=queries.myreservations, description="Reservations made by the current user.")
...
class Mutation:
    create_implementation = strawberry_django.mutation(resolver=mutations.create_implementation, description="Create a new implementation entry.")
    create_foreign_implementation = strawberry_django.mutation(resolver=mutations.create_foreign_implementation, description="Register an external implementation.")
    set_extension_implementations = strawberry_django.mutation(resolver=mutations.set_extension_implementations, description="Set implementations provided by an extension.")
    ack = strawberry_django.mutation(resolver=mutations.ack, description="Acknowledge an assignation.")
    assign = strawberry_django.mutation(resolver=mutations.assign, description="Assign a task to an agent.")
    cancel = strawberry_django.mutation(resolver=mutations.cancel, description="Cancel an active assignation.")
    step = strawberry_django.mutation(resolver=mutations.step, description="Advance an assignation one step.")
    pause = strawberry_django.mutation(resolver=mutations.pause, description="Pause an ongoing assignation.")
    resume = strawberry_django.mutation(resolver=mutations.resume, description="Resume a paused assignation.")
    collect = strawberry_django.mutation(resolver=mutations.collect, description="Collect results from an assignation.")
    interrupt = strawberry_django.mutation(resolver=mutations.interrupt, description="Interrupt the execution of an assignation.")
    reinit = strawberry_django.mutation(resolver=mutations.reinit, description="Reinitialize the assignation or agent.")
    reserve = strawberry_django.mutation(resolver=mutations.reserve, description="Reserve an implementation for future use.")
    unreserve = strawberry_django.mutation(resolver=mutations.unreserve, description="Release a reserved implementation.")
    delete_implementation = strawberry_django.mutation(resolver=mutations.delete_implementation, description="Delete a registered implementation.")
    ensure_agent = strawberry_django.mutation(resolver=mutations.ensure_agent, description="Ensure agent record exists or is up to date.")
    create_test_case = strawberry_django.mutation(resolver=mutations.create_test_case, description="Create a new test case.")
    create_test_result = strawberry_django.mutation(resolver=mutations.create_test_result, description="Create a test result record.")
    shelve_in_memory_drawer = strawberry_django.mutation(resolver=mutations.shelve_in_memory_drawer, description="Shelve data into a memory drawer.")
    ...
    pin_implementation = strawberry_django.mutation(resolver=mutations.pin_implementation, description="Pin an implementation to the user.")
    delete_agent = strawberry_django.mutation(resolver=mutations.delete_agent, description="Delete an agent record.")
...
class Subscription:
    new_actions = strawberry.subscription(resolver=subscriptions.new_actions, description="Subscribe to notifications when new actions are created.")
    assignations = strawberry.subscription(resolver=subscriptions.assignations, description="Subscribe to updates on assignations.")
    reservations = strawberry.subscription(resolver=subscriptions.reservations, description="Subscribe to updates on reservations.")
    assignation_events = strawberry.subscription(resolver=subscriptions.assignation_events, description="Subscribe to events related to assignations.")
    agents = strawberry.subscription(resolver=subscriptions.agents, description="Subscribe to updates on agent connections and statuses.")
    implementation_change = strawberry.subscription(resolver=subscriptions.implementation_change, description="Subscribe to changes in implementations.")
    implementations = strawberry.subscription(resolver=subscriptions.implementations, description="Subscribe to creation or updates of implementations.")
    state_update_events = strawberry.subscription(resolver=subscriptions.state_update_events, description="Subscribe to updates of state values and patches.")
