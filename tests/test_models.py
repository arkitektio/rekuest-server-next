...
from facade.models import Agent, Registry, Action, Protocol, StateSchema, Blok, Dashboard
...
class TestModels:
    ...
    def test_protocol_unique_name_constraint(self):
        ...
        with pytest.raises(IntegrityError):
            Protocol.objects.create(
                name="Unique Protocol",
                description="Second protocol with same name"
            )
    ...
    def test_state_schema_creation(self):
        ...
        ports_data = {
            "input": {"type": "string", "description": "Input port"},
            "output": {"type": "string", "description": "Output port"}
        }

        state_schema = StateSchema.objects.create(
    ...
    def test_state_schema_unique_hash_constraint(self):
        ...
        with pytest.raises(IntegrityError):
            StateSchema.objects.create(
                name="Second Schema",
                hash="unique-hash-456",
                description="Second schema with same hash"
            )
    ...
    def test_blok_creation(self):
        ...
        blok = Blok.objects.create(
            name="Test Blok",
            description="A test UI blok",
            creator=user
        )
