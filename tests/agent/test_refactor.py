...
from tests.factories import build_assignation, build_webhook_agent
...
def test_deliver_to_agent_routes_by_kind(monkeypatch):
    ...
    msg = messages.Cancel(assignation="a1")
...
class TestReconcileOps:
    async def test_reconcile_orphaned_executor_work_is_effect_aware(self):
        none_ass = await build_assignation("rec-none", effect="NONE")
        phys_ass = await build_assignation("rec-phys", effect="PHYSICAL")
        ...
        assert none_ass.latest_event_kind == enums.AssignationEventKind.DISCONNECTED and none_ass.is_done is False
        assert phys_ass.latest_event_kind == enums.AssignationEventKind.CRITICAL and phys_ass.is_done is True

    async def test_reconcile_caller_roots_cancels(self):
        root = await build_assignation("rec-caller", originating_connection_id="connX")
        backend = ModelPersistBackend()

        await backend.reconcile_caller_roots("connX", None)

        root = await type(root).objects.aget(pk=root.pk)
        assert root.is_done is True and root.latest_event_kind == enums.AssignationEventKind.CANCELLED
...
class TestReconcileSweep:
    def test_sweep_fails_stale_disconnected_executor_work(self, settings):
        ...
        ass = async_to_sync(build_assignation)("sweep-stale", effect="NONE")
        ...
        refreshed = Assignation.objects.get(pk=ass.pk)
        assert refreshed.latest_event_kind == enums.AssignationEventKind.DISCONNECTED
    ...
    def test_sweep_leaves_connected_and_webhook_untouched(self, settings):
        ...
        connected = async_to_sync(build_assignation)("sweep-conn", effect="NONE")
        ...
        webhook = async_to_sync(build_assignation)("sweep-hook", effect="NONE")
