...
class TestAgentEvents:
    async def test_log_event_persists(self, agent_ws):
        ...
        await session.send(messages.LogEvent(assignation=str(assignation.pk), message="hello", level="INFO"))
    ...
    async def test_yield_event_persists(self, agent_ws):
        ...
        await session.send(messages.YieldEvent(assignation=str(assignation.pk), returns={"out": 1}))
    ...
    async def test_error_event_marks_assignation_done(self, agent_ws):
        ...
        await session.send(messages.ErrorEvent(assignation=str(assignation.pk), error="boom"))
    ...
    async def test_critical_event_marks_assignation_done(self, agent_ws):
        ...
        await session.send(messages.CriticalEvent(assignation=str(assignation.pk), error="fatal"))
    ...
    async def test_cancelled_event_marks_assignation_done(self, agent_ws):
        ...
        await session.send(messages.CancelledEvent(assignation=str(assignation.pk)))
