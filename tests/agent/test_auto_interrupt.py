...
class TestAutoInterrupt:
    async def test_fires_and_escalates_to_interrupt(self):
        ...
        await backend.on_caller_cancel(agent.pk, messages.CancelRequest(assignation=key, auto_interrupt=0.05))
        ...
        assert enums.AssignationEventKind.CANCELLING in kinds
        assert enums.AssignationEventKind.INTERRUPTING in kinds  # escalated

    async def test_none_disables_escalation(self):
        agent, ass = await _owned("ai-none")
        backend = ModelPersistBackend()
        await backend.on_caller_cancel(agent.pk, messages.CancelRequest(assignation=str(ass.pk)))
    ...
    async def test_confirm_before_window_cancels_timer(self):
        ...
        await backend.on_caller_cancel(agent.pk, messages.CancelRequest(assignation=key, auto_interrupt=30))
        assert key in backend._auto_interrupt
        await backend.on_agent_cancelled(agent.pk, messages.Cancelled(assignation=key))
        assert key not in backend._auto_interrupt  # cancelled on terminal
        assert enums.AssignationEventKind.INTERRUPTING not in await _kinds(ass.pk)
    ...
    async def test_terminal_by_other_path_cancels_timer(self):
        ...
        key = str(ass.pk)
        await backend.on_caller_cancel(agent.pk, messages.CancelRequest(assignation=key, auto_interrupt=30))
        await backend.on_agent_done(agent.pk, messages.Completed(assignation=key))
...
class TestManualEscalation:
    async def test_manual_cancel_then_interrupt(self):
        ...
        await backend.on_caller_cancel(agent.pk, messages.CancelRequest(assignation=key))  # CANCELING, not terminal
        await backend.on_caller_interrupt(agent.pk, messages.InterruptRequest(assignation=key))  # INTERRUPTING

        kinds = await _kinds(ass.pk)
        assert enums.AssignationEventKind.CANCELLING in kinds and enums.AssignationEventKind.INTERRUPTING in kinds

        await backend.on_agent_interrupted(agent.pk, messages.Interrupted(assignation=key))
